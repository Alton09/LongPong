...
public class GameFramework extends View {
    ...
    private float                 drawMin;         // Line length
    ...
    private int                   paddlePos;       // Position in objects
                                                    // ArrayList
    ...
    /**
     * Sets ball and paddle variables based on the screen size.
     */
    private void setDimensions() {
            ...
            ballX = width / 2;
            ballY = height / 2;
            ballWidth = width / 16.667f;
            ballHeight = height / 16.667f;
            ballSpeedX = ballWidth / 30;
            ballSpeedY = ballHeight / 30;
            ...
            if(DEBUG_MODE) {
                Log.i("BALLVARIABLE", "BallX: " + ballX);
                Log.i("BALLVARIABLE", "BallY: " + ballY);
                Log.i("BALLVARIABLE", "BallWidth: " + ballWidth);
                Log.i("BALLVARIABLE", "BallHeight: " + ballHeight);
                Log.i("BALLVARIABLE", "BallSpeedX: " + ballSpeedX);
                Log.i("BALLVARIABLE", "BallSpeedY: " + ballSpeedY);
            }
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if(DEBUG_MODE)
                        Log.i("Fade Thread: ERROR", "Error with thread sleep");
                }
    ...
    private void debug() {
        // Display game objects
         for(int i = 0; i < objects.size(); i++) { GameObject obj =
         objects.get(i); Log.d("Objects","" + obj); }
    ...
    private void createPaddle() {
        ...
        paddlePos = objects.size(); // used to remove paddle in the future
        objects.add(paddle);
    }
    ...
    public void setBallStart() {
        if(DEBUG_MODE)
            Log.i("game.BALLSTART", "ball start with no args");
        ball.setX(ballX);
        ball.setY(ballY);
        ball.setSpeed(ballSpeedX, ballSpeedY);
        ball.setAngle(20);
    ...
    /**
     * Set the ball start position to the location, speed, and angle defined by
     * the parameters x and percY, s, and a respectively.
     * 
     * @param x
     * @param percY
     * @param s
    ...
    public void setBallStart(float x, float percY, Velocity velocity) {
        if(DEBUG_MODE)
            Log.i("game.BALLSTART", "ball start with args- x: " + x + ", percY: "
                    + percY);
        if (clientMode)
    ...
    public synchronized void toggleHasBall() {
        ...
        if(DEBUG_MODE)
            Log.i("TOGGLEBALL", "Ball toggled to: " + hasBall);
    }
